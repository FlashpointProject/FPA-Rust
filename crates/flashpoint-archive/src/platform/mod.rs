...
pub fn find_or_create(conn: &Connection, name: &str) -> Result<Tag> {
    ...
    if let Some(platform) = platform_result {
        ...
        let mut stmt = conn.prepare(
            "INSERT INTO platform_alias (name, platformId) VALUES(?1, ?2) RETURNING id"
        )?;

        // Create a new tag
        let alias_id: i64 = stmt.query_row(params![name, -1], |row| row.get(0))?;
        stmt = conn.prepare(
            "INSERT INTO platform (primaryAliasId, description) VALUES (?1, '') RETURNING id"
        )?;
        let tag_id: i64 = stmt.query_row(params![alias_id], |row| row.get(0))?;

        // Update tag alias with the new tag id
        stmt = conn.prepare(
            "UPDATE platform_alias SET platformId = ?1 WHERE id = ?2"
        )?;
    ...
    match platform_result {
        Ok(mut platform) => {
            let mut platform_alias_stmt = conn.prepare(
                "SELECT pa.name FROM platform_alias pa WHERE pa.platformId = ?1")?;
